...
class BeautifyGenerator extends resourcePolicyVisitor {
  ...
  visitCustom_event(ctx){
    ...
    this.curStateElements.push(`<span class="bp-s-event s-${ruleName}" @click="${eventHandler}">${eventBody}</span>
      </div>`)
...
event_def
  ...
  .forEach((event, i) => {
    ...
    BeautifyGenerator.prototype[fnName] = function (ctx){
      ...
      this.curStateElements.push(`<span class="bp-s-event bp-s-${ruleName}" @click="${eventHandler}">${eventBody}</span>
      </div>`)
